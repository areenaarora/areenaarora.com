...
<script lang="ts">
	...
	const funProjects: Experiment[] = [
		{
			...
			summary:
				'Dice, strategy, and high-score loops designed for quick replay. Imported from the Projects window on /fun.',
			status: 'live',
			kind: 'game',
			difficulty: 'medium',
			tech: [],
		...
		},
		{
			...
			summary: 'The Windows-inspired desktop interface that powers the original /fun page.',
			status: 'building',
			kind: 'ui',
			difficulty: 'easy',
			tech: [],
	...
	$: ticker = [
		...new Set([
			'FROM /FUN PROJECTS',
			'FROM /FUN BLOG',
			...experiments.slice(0, 6).map((exp) => exp.title.toUpperCase())
		])
	];
	...
	$: filtered = experiments
		...
		.filter((exp) => {
			...
			const hay = `${exp.title} ${exp.summary} ${exp.kind} ${exp.status} ${exp.tech.join(' ')}`.toLowerCase();
	...
	$: if (isbnDigits(currentBook.isbn) !== activeBookIsbn) {
		...
		bookCoverUrl = getBookCoverProxyUrl(currentBook.isbn);
	}
</script>
...
<main class="fun2">
	<div class="page-grid">
		<aside class="sidebar" aria-label="Currently">
			...
			<section class="status-item">
				<p class="status-label">Building</p>
				<a class="mini-block" href={heroSlabs[2].link}>
					<p class="media-title small">{heroSlabs[2].title}</p>
					<p class="media-detail">{heroSlabs[2].detail}</p>
				</a>
			</section>
		</aside>
		...
		<section class="content" aria-label="Archive and activity">
			...
			<div class="controls" aria-label="Sort and filters">
				<label>
					<span>Search</span>
					<input type="text" bind:value={search} placeholder="Search experiments" />
				</label>
